...
int main(int argc, char *argv[])
{
  ...
  int show_spectrum=0;
  int show_iq=0;
  int show_equalizer=0;

  while ((opt = getopt(argc, argv, "SIQ")) != -1) {
      switch (opt) {
      case 'S': show_spectrum = 1; break;
      case 'I': show_iq = 1; break;
      case 'Q': show_equalizer = 1; break;
      default:
          fprintf(stderr, "Usage: %s [-SIQ] [file...]\n", argv[0]);
          exit(EXIT_FAILURE);
      }
  }
  ...
  if(show_spectrum) {
	  fprintf(stdout, "spectrum view enabled!\n");
    if((ret = drmplusSetCallback(h, SIGSP_CALLBACK, spectrum_update, NULL)) != DRMPLUS_OK) {
	  fprintf(stdout, "setting spectrum callback failed: %d!\n", ret);
	  return -1;
    }
  }

  if(show_iq) {
	  fprintf(stdout, "IQ view enabled!\n");
    if((ret = drmplusSetCallback(h, SIGIQ_CALLBACK, iq_update, NULL)) != DRMPLUS_OK) {
	  fprintf(stdout, "setting spectrum callback failed: %d!\n", ret);
	  return -1;
    }
  }
  ...
  if(show_equalizer) {
	  fprintf(stdout, "Equalizer view enabled!\n");
    if((ret = drmplusSetCallback(h, SIGEQ_CALLBACK, equalizer_update, NULL)) != DRMPLUS_OK) {
	  fprintf(stdout, "setting spectrum callback failed: %d!\n", ret);
	  return -1;
    }
  }
  ...
  drmplusSetCallback(h, STREAM0_CALLBACK, strX_update, NULL);
  drmplusSetCallback(h, STREAM1_CALLBACK, strX_update, NULL);
  drmplusSetCallback(h, STREAM2_CALLBACK, strX_update, NULL);
  drmplusSetCallback(h, STREAM3_CALLBACK, strX_update, NULL);
