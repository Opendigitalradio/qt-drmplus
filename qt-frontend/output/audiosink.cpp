...
int32_t	i;
	...
	if (Pa_Initialize () != paNoError) {
       qDebug("Initializing Pa for output failed");
	   return;
	}
...
int16_t	outputDevice;
	...
	if (!isValidDevice (outputDevice)) {
       qDebug("invalid device (%d) selected", outputDevice);
	   return false;
	}
...
//
    qDebug("Suggested size for outputbuffer = %d", bufSize);
...
uint16_t	i;
	...
	for (i = 0; i <  numofDevices; i ++) {
	   ...
	   if (so != QString ("")) {
	      ...
	      outTable [ocnt] = i;
          qDebug (" (output):item %d wordt stream %d (%s)", ocnt , i,
	                      so. toLatin1 ().data ());
	      ocnt ++;
	   }
	}
