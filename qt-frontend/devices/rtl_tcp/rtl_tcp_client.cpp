...
#define	DEFAULT_FREQUENCY	(Khz (220000))
    ...
    rtl_tcp_client::rtl_tcp_client	(QSettings *s, QFrame *fr) {
	remoteSettings		= s;

    theFrame		= new QFrame(fr);
	setupUi (theFrame);
	this	-> theFrame	-> show ();
...
//	   streamer. close ();
	...
	delete	hostLineEdit;
    //delete	theFrame;
    qDeleteAll(theFrame->findChildren<QWidget*>("", Qt::FindDirectChildrenOnly));
    theFrame->deleteLater();
}
//
